        ...
        if request.user:
            ...
            if request.GET.get('type') == 'week':
                ...
                for x in range(0, days):
                    ...
                    datetime_list.append(new_time)
                    labels.append(new_time.strftime("%a"))
                    new_qs = qs.filter(updated__day=new_time.day,updated__month=new_time.month).not_created().filter(cart__cartitem__product__user=self.request.user).distinct()
            ...
            if request.GET.get('type') == '4weeks':
                ...
                data['data'] = []
                for i in range(0, 5):
                    new_qs = qs.by_weeks_range(weeks_ago=current, number_of_weeks=1).not_created().filter(cart__cartitem__product__user=self.request.user).distinct()
...
class Supplier_SalesView(LoginRequiredMixin,TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        # two_weeks_ago = timezone.now() - datetime.timedelta(days=14)
        # one_week_ago = timezone.now() - datetime.timedelta(days=7)
        qs = Order.objects.all().by_weeks_range(weeks_ago=10, number_of_weeks=10).filter(cart__cartitem__product__user=self.request.user).distinct()   #(start_date=two_weeks_ago, end_date=one_week_ago)
        start_date = timezone.now().date() #- datetime.timedelta(hours=24)
        end_date = timezone.now().date() + datetime.timedelta(hours=24)
